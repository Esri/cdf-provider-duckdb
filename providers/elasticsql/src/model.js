...
class Model {
  ...
  constructor({ logger }, {}) {
    this.#logger = logger;
    const elasticConfig = koopConfig.elasticsql.sources.fires;

    // Validate registration options
    try {
      validateConn(elasticConfig.conn);

      if (elasticConfig.idFieldMap) {
        validateIdFieldMap(elasticConfig.idFieldMap);
      }

      if (elasticConfig.geometryFieldMap) {
        validateGeometryFieldMap(elasticConfig.geometryFieldMap);
      }
    } catch (error) {
    ...
    this.#client = new Client(elasticConfig.conn);

    // define a lookup to find the geometry field for a given elastic index
    this.#geometryFieldMap = elasticConfig.geometryFieldMap || {};

    // define a lookup to find the id-field for a given elastic index
    this.#idFieldMap = elasticConfig.idFieldMap || {};
  }
